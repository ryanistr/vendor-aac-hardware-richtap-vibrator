...
interface IVibrator {
    /**
     * Whether on w/ IBinder can be used w/ 'on' function
     */
    const int CAP_ON_CALLBACK = 1 << 0;
    /**
     * Whether on w/ IBinder can be used w/ 'perform' function
     */
     ...
     */
    void on(in int timeoutMs, in IBinder callback);
     ...
     *     approximation.
     */
    int perform(in Effect effect, in EffectStrength strength, in IBinder callback);
     ...
     *
     * Doing this operation while the vibrator is already on is undefined behavior. Clients should
     * explicitly call off. IBinder.onComplete() support is required for this API.
     *
     * @param composite Array of composition parameters.
     */
    void compose(in CompositeEffect[] composite, in IBinder callback);
     ...
     */
    void composePwle(in PrimitivePwle[] composite, in IBinder callback);
}
